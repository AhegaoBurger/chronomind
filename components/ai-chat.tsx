...
export function AIChat({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hi there! I'm your AI time management assistant. How can I help you organize your schedule today?",
    },
  ]);
  ...
  const [currentSuggestion, setCurrentSuggestion] = useState<Activity | null>(
    null,
  );
  const [suggestionMessageId, setSuggestionMessageId] = useState<string | null>(
    null,
  );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.suggestedActivity) {
          const { title, start, end, description, categoryId } =
          ...
          if (title && start && end && categoryId) {
            ...
            if (!isNaN(startDate.getTime()) && !isNaN(endDate.getTime())) {
              ...
              console.warn(
                "Received suggestedActivity with invalid date(s):",
                data.suggestedActivity,
              );
            }
          } else {
            console.warn(
              "Received suggestedActivity with missing required fields:",
              data.suggestedActivity,
            );
          }
        }
      ...
      const assistantMessage: Message = {
        ...
        role: "assistant",
        content:
          "Sorry, there was an issue reaching the AI assistant. Please check your connection and try again.",
      };
      setMessages((prev) => [...prev, assistantMessage]);
    } finally {
      setIsLoading(false);
                        ...
                        <p className="text-sm font-medium mb-2">
                          ...
                          {currentSuggestion.description && (
                            <>
                              <br />
                              <span className="text-xs">
                                Details: {currentSuggestion.description}
                              </span>
                            </>
                          )}
                        </p>
